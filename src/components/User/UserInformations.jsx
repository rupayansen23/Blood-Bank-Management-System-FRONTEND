...
export default function UserInformations() {
  ...
  const allKeys = Object.keys(donorBesicInfo || {}).filter((k) => k !== "password");
  const orderedKeys = [
    ...preferredOrder.filter((k) => allKeys.includes(k)),
    ...allKeys.filter((k) => !preferredOrder.includes(k)),
  ];

  const visibleKeys = orderedKeys.filter((k) => !isEmptyValue(donorBesicInfo?.[k]));
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <h1 className="m-3 p-2 text-2xl font-semibold">Your Information</h1>
              ...
              {visibleKeys.map((key) => (
                <tr key={key} className="odd:bg-gray-50">
                  ...
                  <td className="w-2/3 px-6 py-4 text-gray-800">
                    {makeDisplayValue(donorBesicInfo[key])}
                  </td>
                </tr>
              ))}
            </tbody>
